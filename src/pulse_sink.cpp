...
size_t PulseAudioSink::write(const uint8_t* data, size_t length, uint32_t timeout_ms) {
    ...
    // deadlock.
    //
    // One report for the gap and this write together: the player sums the frames of reports it
    // has not read yet but keeps only the last timestamp, so a separate gap report could lose it.
