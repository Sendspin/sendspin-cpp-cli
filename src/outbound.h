...
/// origin. Every dial goes to a discovered server, and the mDNS instance label *is* the
/// protocol server_id, so a URL is answered only for the server it dialled. Losing the
/// connection forgets the dial: whatever its URL described is gone, and no answer beats a
/// stale one.
class LastDial {
public:
    /// @brief Records that connect_to() has just been called with `url`.
    /// @param server_id The id of the discovered server that was dialled.
    ...
    /// server other than the connected one, an unknown one or no id at all included.
